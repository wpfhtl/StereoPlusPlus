						...
						std::vector< std::vector<int> >& boundaryLabels)
{
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			double estimatedDisparity = initialDisparityImage_[width_*y + x];

			disparityImage.set_pixel(x, y, static_cast<unsigned short>(estimatedDisparity*outputDisparityFactor_ + 0.5));
	...
	disparityImageRight.resize(width_, height_);
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			double estimatedDisparity = initialDisparityImageRight_[width_*y + x];

			disparityImageRight.set_pixel(x, y, static_cast<unsigned short>(estimatedDisparity*outputDisparityFactor_ + 0.5));
			if (estimatedDisparity <= 0.0 || estimatedDisparity > 255.0) {
				disparityImageRight.set_pixel(x, y, 0);
			}
			else {
				disparityImageRight.set_pixel(x, y, static_cast<unsigned short>(estimatedDisparity*outputDisparityFactor_ + 0.5));
			}
...
void SPSStereo::allocateBuffer() {
	...
	initialDisparityImageRight_ = reinterpret_cast<float*>(malloc(width_*height_*sizeof(float)));
...
void SPSStereo::freeBuffer() {
	...
	free(initialDisparityImageRight_);
	free(labelImage_);
...
void SPSStereo::computeInitialDisparityImage(const png::image<png::rgb_pixel>& leftImage, const png::image<png::rgb_pixel>& rightImage) {
	SGMStereo sgm;
	sgm.compute(leftImage, rightImage, initialDisparityImage_, initialDisparityImageRight_);
}

...
int SGMSTEREO_DEFAULT_DISPARITY_TOTAL = 256;
...
SGMStereo::SGMStereo() : disparityTotal_(SGMSTEREO_DEFAULT_DISPARITY_TOTAL),
						 ...
						 censusWeightFactor_(ARAP_CENSUS_WEIGHT),
...
void SGMStereo::compute(const png::image<png::rgb_pixel>& leftImage,
						...
						float* disparityImageL, float *disparityImageR)
{
	...
	//FILE *fid = fopen("D:\\data\\KITTI\\myresults\\Yamaguchi_CostVolume.bin", "wb");
	//fwrite(leftCostImage_, sizeof(unsigned short), width_*height_*disparityTotal_, fid);
	//fclose(fid);
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			disparityImageL[width_*y + x] = static_cast<float>(leftDisparityImage[width_*y + x]/disparityFactor_);
		}
	}
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			disparityImageR[width_*y + x] = static_cast<float>(rightDisparityImage[width_*y + x] / disparityFactor_);
		}
	}
...
{
	png::image<png::rgb_pixel> pngImageL(filePathImageL);
	png::image<png::rgb_pixel> pngImageR(filePathImageR);
	
	int numRows = pngImageL.get_height();
	int numCols = pngImageL.get_width();
	...
	gNumDisps += 15 - (gNumDisps - 1) % 16;
	extern int SGMSTEREO_DEFAULT_DISPARITY_TOTAL;
	extern double SGMSTEREO_DEFAULT_DISPARITY_FACTOR;
	SGMSTEREO_DEFAULT_DISPARITY_TOTAL = gNumDisps;
	SGMSTEREO_DEFAULT_DISPARITY_FACTOR = (benchmark == "Midd3" ? 64 : 256);
	float upFactor = SGMSTEREO_DEFAULT_DISPARITY_FACTOR;
